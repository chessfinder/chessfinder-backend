...
import zio.{ IO, ZIO }
import search.entity.{UserName, User}
...
object BrokenLogic:
  ...
  case class ProfileIsNotCached(user: User) extends BrokenLogic(s"Profile ${user.userName} from ${user.platform.toString} is not chached!")
  case class TaskProgressOverflown(taskId: TaskId) extends BrokenLogic(s"Task ${taskId.toString} progerss is overflown")
