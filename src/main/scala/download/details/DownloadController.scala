...
class DownloadController(val version: String) extends ZTapir:
  ...
  val `GET /api/version/game` =
    baseUrl.get
      ...
      .errorOut(jsonBody[ApiError]) // fixme those errors should go with 422, other errors caused by missing params have 400 and are just a text not json, the client should be able to distinguish using status code. 422s are always json and mean some business error. 
