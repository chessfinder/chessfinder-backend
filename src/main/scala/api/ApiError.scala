...
object ApiError:
  ...
  def fromBrokenLogic(err: BrokenComputation): ApiError = err match
    ...
    case err: BrokenComputation.TaskNotFound    => ApiError("TASK_NOT_FOUND", err.msg) //FIXME change TASK to something else
    ...
    case err                                    => ApiError("SERVER_OVERLOADED", err.msg) //FIXME err.msg is a bug, you are not supposed to reveal the real cause
